...
function getHash(client_token, server_token, password, cb) {
   ...
   */
  const a = Bridge.assembly("Demo", function ($asm, globals) {
    "use strict";

    Bridge.define("Demo.BattleNet.Bsha1", {
      main: function Main() {

        cb(null, new Buffer(System.BitConverter.toString(Demo.BattleNet.Bsha1.DoubleHash(client_token, server_token, password).toArray()).replace(/-/g,''),'hex'));
      },
      statics: {
        methods: {
