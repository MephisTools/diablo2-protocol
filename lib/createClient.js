...
function createClient ({ username, password, host, port, character, gameName, gamePassword, gameServer }) {
  ...
  clientSid.on('SID_AUTH_INFO', ({ logonType, serverToken, udpValue, mpqFiletime, mpqFilename, valuestring }) => {
    ...
    clientDiablo.setClientBnftp(getMpq(host, port, mpqFiletime, mpqFilename, clientSid.platformId, clientSid.productId, writeAuthCheck))
  })
  ...
  clientSid.on('SID_LOGONREALMEX', ({ MCPCookie, MCPStatus, MCPChunk1, IP, port, MCPChunk2, battleNetUniqueName }) => {
    ...
    clientDiablo.selectCharacter = (character, cb) => {
    ...
    clientDiablo.on('charactersList', characters => {
      clientDiablo.selectCharacter(clientSid.character, () => {
    ...
    function createD2gs () {
      ...
      */
      clientD2gs.on('D2GS_PLAYERMOVE', ({ targetX, targetY }) => {
