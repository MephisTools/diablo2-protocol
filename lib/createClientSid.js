...
function createClientSid (clientDiablo, host) {
  const portSid = 6112
  return new Promise((resolve) => {
    ...
    clientDiablo.on('SID_AUTH_INFO', ({ logonType, serverToken, udpValue, mpqFiletime, mpqFilename, valuestring }) => {
      ...
      clientDiablo.setClientBnftp(getMpq(host, portSid, mpqFiletime, mpqFilename, clientDiablo.platformId, clientDiablo.productId, writeAuthCheck))
    })
    ...
    clientDiablo.on('SID_LOGONREALMEX', ({ MCPCookie, MCPStatus, MCPChunk1, IP, port, MCPChunk2, battleNetUniqueName }) => {
      clientDiablo.MCPCookie = MCPCookie
      const hostMcp = IP[0] + '.' + IP[1] + '.' + IP[2] + '.' + IP[3]
      resolve({ hostMcp, portMcp: port, MCPCookie, MCPStatus, MCPChunk1, MCPChunk2, battleNetUniqueName })
    })
    clientSid.connect()
  })
}
