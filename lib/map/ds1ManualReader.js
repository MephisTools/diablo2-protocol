...
class Ds1 {
  ...
  static loadBytes (basePath, bytes) {
    ...
    offset = Ds1.readObjects(basePath, ds1, bytes, offset, act)
  ...
  static async readObjects (basePath, ds1, bytes, offset, act) {
    if (ds1.version < 2) {
      return offset
    }
    ...
    */
    ...
    for (let i = 0; i < objectCount; i++) {
      const info = {}
      /* const type = */bytes.readInt32LE(offset) // type
      offset += 4
      ...
      info.preset = {}
      info.preset = json.find(j => {
        return j['Act'].includes(act) && j['Id'].includes(id) /* && j['SizeX'].includes(info.x) && j['SizeY'].includes(info.y) */
      })
